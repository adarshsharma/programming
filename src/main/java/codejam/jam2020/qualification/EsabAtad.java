...
public class EsabAtad {
  ...
  public static void solve(Scanner input, int B) {
    try (BufferedWriter bw = new BufferedWriter(new FileWriter(
        "/Users/sharmaadarsh/Documents/programming/src/main/java/codejam/jam2020/qualification/out"))
    ) {
      char[] arr = new char[B];
      int[] prev = new int[4];
      arr[0] = read(input, 0, bw);
      arr[B - 1] = read(input, B - 1, bw);
      prev[0] = arr[0];
      prev[3] = arr[B - 1];
      ...
      for (int i = 1; i < 5; i++) {
        arr[i] = read(input, i, bw);
        arr[B - 1 - i] = read(input, B - 1 - i, bw);
      ...
      while (read < B) {
        Action action = getAction(input, leftMarker, rightMarker, prev, foundSame, B, bw);
        query += 2;
        if (leftMarker != -1) {
          query += 2;
        }
        ...
        for (int i = 0; i < limit && read < B && query <= 150; i++) {
          arr[left + i] = read(input, left + i, bw);
          arr[right - i] = read(input, right - i, bw);
  ...
  private static Action getAction(Scanner input, int leftMarker, int rightMarker, int[] prev,
      boolean foundSame, int B, BufferedWriter bw) throws IOException {
    int[] now = new int[4];
    now[0] = read(input, 0, bw);
    now[3] = read(input, B - 1, bw);
    Action action;

    if (leftMarker != -1) {
      now[1] = read(input, leftMarker, bw);
      now[2] = read(input, rightMarker, bw);
  ...
  private static char read(Scanner input, int idx, BufferedWriter bw) throws IOException {
